...
export default function DonorsPage() {
  ...
  const totalDonated    = donors.reduce((s, d) => s + d.totalDonated, 0)
  const totalSponsored  = donors.reduce((s, d) => s + d.activeStudents, 0)
            ...
            <TableBody>
              {filtered.map((d) => (
                <TableRow key={d.id}>
                  ...
                  <TableCell className="hidden sm:table-cell font-medium text-primary">{fmt(d.totalDonated)}</TableCell>
                  <TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} disabled={!form.name || !form.contact}>
              {editTarget ? "Save Changes" : "Add Donor"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
